...
export default async function migrate(stateAsync: unknown) {
  ...
  if (!isObject(state)) {
    captureException(
      new Error(`Migration 60: Invalid root state: '${typeof state}'`),
    );
    return state;
  }
  ...
  if (!isObject(state.engine) || !hasProperty(state, 'engine')) {
    captureException(
      new Error(
        `Migration 60: Invalid or missing 'engine' in state: '${typeof state.engine}'`,
      ),
    );
    return state;
  }
  ...
  if (
    ...
    captureException(
      new Error(
        `Migration 60: Invalid or missing 'backgroundState' in engine: '${typeof state
  ...
  if (
    ...
    captureException(
      new Error(
        `Migration 60: Invalid or missing 'NetworkController' in backgroundState: '${typeof networkControllerState}'`,
      ),
    );
    return state;
  }
  ...
  if (
    ...
    captureException(
      new Error(
        `Migration 60: Missing or invalid 'selectedNetworkClientId': '${networkControllerState.selectedNetworkClientId}'`,
      ),
    );
    return state;
  }
  ...
  if (
    ...
    captureException(
      new Error(
        `Migration 60: Missing or invalid 'networkConfigurationsByChainId' in NetworkController`,
      ),
    );
    return state;
  }
  ...
  for (const chainId in networkConfigurationsByChainId) {
    ...
    if (
      ...
      captureException(
        new Error(
          `Migration 60: Invalid network configuration or missing 'rpcEndpoints' for chainId: '${chainId}'`,
        ),
      );
    }
  }
